...
if ! node --test --test-concurrency=1 tests/node/stream-tool-sieve.test.js tests/node/chat-stream.test.js tests/node/chat-history-utils.test.js tests/node/usage-ledger-utils.test.js tests/node/js_compat_test.js webui/src/theme.test.mjs "$@" 2>&1 | tee "$NODE_TEST_LOG"; then
  echo
